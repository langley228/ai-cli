...
# /dev 特例：放行常見偽裝置（/dev/null、重導向、亂數源…），這些是日常
# 指令的合法用法（如 `cmd > /dev/null`），不該誤擋；其餘（如原始磁碟
# /dev/sda、/dev/mem）才視為危險。做法：先把安全偽裝置字樣抽掉，若仍
# 殘留 /dev/ 才算命中。
...
if [ "$BLOCKED" -eq 1 ]; then
  cat <<'JSON'
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"protect-system-paths hook：偵測到對系統檔案目錄（/etc /proc /sys /boot /root）或危險裝置檔（/dev/ 下非偽裝置）的存取，已阻擋。"}}
JSON
fi
